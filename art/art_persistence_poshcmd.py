...
class Atomic(Atomic):
    ...
    def payload(self):
        with open(get_path_in_package('core/srv/ttp/art/src/hybrid.boo'), 'r') as ttp_src:
            src = ttp_src.read()
            hybrid_script = get_path_in_package(f'core/srv/ttp/art/posh_cmd_scripts/persistence/{self.options["Atomic"]["Value"]}')
    ...
    def walking_in_directory(self):
        path = get_path_in_package('core/srv/ttp/art/posh_cmd_scripts/persistence/')
