...
class Atomic(Atomic):
    ...
    def payload(self):
        with open(get_path_in_package('core/srv/ttp/art/src/powershell.boo'), 'r') as ttp_src:
            src = ttp_src.read()
            posh_script = get_path_in_package(f'core/srv/ttp/art/ps1_scripts/credentialAccess/{self.options["Atomic"]["Value"]}')
    ...
    def walking_in_directory(self):
        path = get_path_in_package('core/srv/ttp/art/ps1_scripts/credentialAccess/')
